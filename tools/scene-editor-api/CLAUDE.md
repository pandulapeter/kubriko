...
```kotlin
EditableMetadata.create<MyActor, MyActor.State>(typeId = "MyActor") { position ->
    MyActor.State(body = BoxBody(initialPosition = position))
}
```
- `typeId` is required and must be a string literal. It is persisted into scene files, so it cannot be derived from `T::class.simpleName`: R8/ProGuard renames the actor class, the derived id stops matching what the committed scene files store, and every actor is silently dropped at load time. The examples pass their actor's class name explicitly.
