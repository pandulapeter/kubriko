...
private fun HexInput(
    ...
    val hexText = value.toHexString()
    var textFieldValue by remember(hexText) { mutableStateOf(TextFieldValue(hexText, TextRange(hexText.length))) }
    var isFocused by remember { mutableStateOf(false) }
    val focusReporter = LocalTextInputFocusReporter.current
    DisposableEffect(Unit) {
        onDispose { if (isFocused) focusReporter(false) }
    }
    BasicTextField(
        modifier = modifier.onFocusChanged { state ->
            if (state.isFocused != isFocused) {
                isFocused = state.isFocused
                focusReporter(state.isFocused)
            }
        },
