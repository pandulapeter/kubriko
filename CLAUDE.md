...
- **User-facing strings**: every string shown on screen lives in a `composeResources/values/strings.xml` and is read via `stringResource(Res.string.…)` — never inline string literals in UI code. Use parameterized resources for strings built from dynamic parts. See the `code-style` skill for the full rule (keys, exemptions, and the `StringResource`-at-call-site pattern for enums/factory lambdas).
